...
## Governance & Audit > Resource Watcher > 콘솔 사용 가이드 { #governance-audit-resource-watcher-console-guide }
...
| 항목                | 설명                                                                                                                                        |
|-------------------|-------------------------------------------------------------------------------------------------------------------------------------------|
| 리소스 태그 검색 조건      | 다음과 같은 검색 조건을 지정하여 해당하는 리소스 태그 목록을 검색할 수 있습니다.<li>리소스 태그 구분: USER(사용자 유형), SYSTEM(시스템 유형)</li><li>리소스 태그 키: 입력한 이름을 포함하는 리소스 태그 선택</li> |
| 리소스 태그 생성 및 삭제    | 새로운 리소스 태그를 추가하거나 목록에 표시된 리소스 태그를 선택하여 삭제할 수 있습니다.                                                                                        |
| 리소스 태그 전체 선택      | 화면에 표시된 모든 리소스 태그를 선택하거나, 선택을 해제합니다.                                                                                                      |
| 리소스 태그 항목 표시      | 리소스 태그의 이름, 구분, 최종 수정 일시 정보를 화면에 표시합니다. 각각의 리소스 태그를 선택하여 수정 및 삭제할 수 있습니다.                                                                 |
| 리소스 태그별 리소스 목록 조회 | 리소스 태그가 설정되어 있는 리소스 목록을 조회할 수 있습니다.                                                                                                       |
| 태그 수정             | 리소스 태그 키의 이름 또는 리소스 태그 키에 속한 리소스 태그 값을 변경할 수 있습니다.                                                                                        |
...
| 리소스 목록 | 선택한 리소스 태그의 리소스 목록을 표시합니다. |

> 리소스 태그 검색 화면에서는 리소스 태그를 설정한 리소스 목록에 대한 조회만 제공합니다.
> 리소스 태그 키 뿐만 아니라 태그 키에 속한 리소스 태그 값이 설정된 리소스 목록이 모두 조회됩니다.
...
리소스 태그 생성 화면에서 추가할 태그 키와 태그 값을 입력하고 **추가**을 누르면 리소스 태그가 생성됩니다.
리소스 태그 키의 이름은 중복될 수 없으며 리소스 태그 값은 태그 키 내에서 중복될 수 없습니다.

<a id="resource-tag-modify-resource-tag"></a>
#### 리소스 태그 수정

리소스 태그 검색 화면의 리소스 태그 목록에서 변경할 항목에 대해 **태그 수정**을 누릅니다.

![resourcewatcher_13](https://static.toastoven.net/prod_resource_watcher/img_20240723_modify_tags_KO.png)

태그 키를 수정할 수 있으며, 태그 키에 속한 태그 값을 변경할 수 있습니다.
...
| 프로젝트 | 리소스가 생성된 프로젝트 이름.<br>조직 서비스의 리소스이면 해당 정보를 표시하지 않음.                                                                                                                                                                                          |
| 서비스 | 리소스가 속한 서비스 정보.                                                                                                                                                                                                                           |
...
| 항목         | 설명                                                                                                                                                                  |
|------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 리소스 검색 조건  | 다음과 같은 검색 조건을 지정하여 해당하는 리소스를 검색할 수 있습니다.<li>프로젝트명</li><li>서비스</li><li>리소스 유형</li><li>리전</li><li>리소스 그룹</li><li>리소스 상태</li><li>리소스 태그</li><li>리소스명</li><li>리소스 생성 일시 |
| 리소스 태그 재설정 | 선택한 리소스에 리소스 태그를 설정합니다.                                                                                                                                             |
...
| 상세보기       | 해당 리소스에 대한 **기본 정보**와 **이벤트 조회**를 확인합니다.                                                                                                                                   |
...
리소스에 리소스 태그를 설정하려면 리소스 검색 화면에서 **리소스 태그 재설정**을 클릭합니다.

![resourcewatcher_45](https://static.toastoven.net/prod_resource_watcher/img_20240723_reset_resource_tags_KO.png)

| 항목           | 설명                                                                       |
|--------------|--------------------------------------------------------------------------|
| 리소스 선택       | 리소스 태그를 설정하려는 리소스에 대한 체크박스를 선택합니다. 체크박스를 선택하면 **리소스 태그 재설정** 버튼이 활성화됩니다. |
| 리소스 태그 재설정   | **리소스 태그 재설정**을 누르면 설정 화면이 팝업됩니다.                                        |
| 태그 키         | 설정할 수 있는 리소스 태그 키 목록이 조회됩니다. 검색을 통해 설정할 태그 키 목록을 조회할 수 있습니다.             |
| 태그 키: 태그 값   | 설정할 수 있는 리소스 태그 키에 속한 태그 값 목록이 조회됩니다. 검색을 통해 설정할 태그 값 목록을 조회할 수 있습니다.    |
| 선택 가능한 항목 표시 | 모든 리소스 태그 목록을 표시합니다. 하나의 리소스에 동일한 태그 키를 가진 태그 값은 동시에 설정할 수 없습니다.         |
| 확인           | **리소스 태그 재설정** 화면의 내용을 반영합니다.                                            |

리소스 태그 재설정 시 주의사항
> 리소스 태그를 재설정하면 기존에 설정되었던 리소스 태그 목록이 모두 초기화되고 새롭게 지정한 리소스 태그로 설정됩니다.

<a id="resource-tag-setting"></a>
#### 리소스 태그 설정

개별 리소스 조회 후 리소스 태그 항목의 **설정** 을 클릭합니다.
...
| 태그 키: 태그 값      | 설정할 수 있는 리소스 태그 키에 속한 태그 값 목록이 조회됩니다. 검색을 통해 설정할 태그 값 목록을 조회할 수 있습니다. |
| 선택 가능한 항목 표시    | 모든 리소스 태그 목록을 표시합니다. 하나의 리소스에 동일한 태그 키를 가진 태그 값은 동시에 설정할 수 없습니다.      |
| 선택한 태그 키: 태그 값  | 리소스에 기존 선택된 리소스 태그 목록을 표시합니다.                                         |
